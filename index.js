...
function createSymbolWithDescription(property) {
  ...
  const swd = Symbol(property);
  //Повертаємо опис символа symbol
  return swd.description;
}
...
function createSymbolForKey(key) {
  ...
  const freshSym = Symbol.for(key);
  // Використовуємо Symbol.for() для створення нового символу symbol зі значенням key  Symbol.for() шукає символ в глобальному реєстру символів.
